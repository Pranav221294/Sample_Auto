...
static void r_iic11_callback_master_sendend(void)
{
	...
	eI2CState eI2CCurState;
	eI2CCurState = eGetI2CBusState();

	if (eI2CCurState == eI2CAddressSent)
		vSetI2CBusState(eI2CReadDataTransmit);
	else if (bIsReadFromSlave() == TRUE)
		vSetI2CBusState(eI2CRead);
	else
		vSetI2CBusState(eI2CIdle);    
